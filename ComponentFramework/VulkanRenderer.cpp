...
void VulkanRenderer::createGraphicsPipeline(const char *vertexShader_, const char *fragShader_) {
    auto vertShaderCode = readFile(vertexShader_);
...
void VulkanRenderer::SetUBO(const Matrix4& projection_, const Matrix4& view_, const Matrix4& model_, const Vec4 &lightPosition_)
{
    ubo.proj = projection_;
    ubo.view = view_;
    ubo.model = model_;
    ubo.lightPos = lightPosition_;
    ubo.proj[5] *= -1.0f;
}
